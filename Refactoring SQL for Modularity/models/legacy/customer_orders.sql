...
from sk_raw.jaffle_shop.orders as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from sk_raw.jaffle_shop.customers
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from sk_raw.jaffle_shop.orders
    ) a
    ...
    left outer join sk_raw.stripe.payment c
    on a.id = c.orderid
...
left outer join sk_raw.stripe.payment payments
